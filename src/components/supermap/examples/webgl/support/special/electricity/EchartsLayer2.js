!(function () {
    ...
    function registerMap(layer) {
        if (!SuperMap3D.defined(echarts)) {
            throw new SuperMap3D.DeveloperError("echarts is undefined.");
        }
    ...
    function createChart(layer) {
        ...
        ele.style.height = window.screen.height + "px";
    ...
    function getCoordinateSystem(viewer) {
        function GLMapCoordSys(api) {
			...
			var camerapos = this._viewer.scene.camera.position;
			var convertcamerapos = SuperMap3D.SceneTransforms.convert2DToCartesian(this._viewer.scene, camerapos);
			
            this._occluder = new SuperMap3D.EllipsoidalOccluder(this._viewer.scene.globe.ellipsoid, convertcamerapos);
        }
        ...
        GLMapCoordSys.prototype.dataToPoint = function (data) {
            var e = [0, 0],
                //i = SuperMap3D.Cartesian3.fromDegrees(data[0], data[1]);
				pos = new SuperMap3D.Cartesian3(data[0], data[1], 0);
				//var convertposition = SuperMap3D.SceneTransforms.convert2DToCartesian(scene, position);
				i = SuperMap3D.SceneTransforms.convert2DToCartesian(this._viewer.scene,pos);
				// alert(i)
            if (!i) return e;
			
			var camerapos = this._viewer.scene.camera.position;
			var convertcamerapos = SuperMap3D.SceneTransforms.convert2DToCartesian(this._viewer.scene, camerapos);
            this._occluder.cameraPosition = convertcamerapos;
            if(!this._occluder.isPointVisible(i)) {
                return [];
            };
            var n = viewer.scene.cartesianToCanvasCoordinates(i);
            if (!n) return e;
            return (SuperMap3D.Cartesian3.angleBetween(viewer.scene.camera.position, i) < SuperMap3D.Math.toRadians(145))
        ...
        GLMapCoordSys.prototype.pointToData = function (pt) {
            ...
            var cart = viewer.scene.pickPosition(
                new Cartesian2(pt[0] + mapOffset[0], pt[1] + mapOffset[1]), new SuperMap3D.Cartesian3()
            );
            var carto = SuperMap3D.Cartographic.fromCartesian(cart);
            return [SuperMap3D.Math.toDegrees(carto.longitude), SuperMap3D.Math.toDegrees(carto.latitude)];
        }
